<?php include "header.php" ?>
<!-- comment -->
<!-- test -->
<?php include "product-script.php" ?>

<div class="product-description">
	<div class="container">
		<div class="col-sm-6 col-xs-12">
			<h3>Broccoli & Carrot Khichdi</h3>
			<ul class="product-content">
				<li>215g</li>
				<li>Coarse</li>
				<li>Meal</li>
			</ul>
			<p>Traditional khichdi with a healthy twist – powered with
			the goodness of broccoli, peas and carrots and flavoured
			by aromatic spices, this makes for a wholesome, nutritious
			meal for your child.</p>
			<button type="button" class="btn btn-primary snipcart-add-item"
			 data-item-name="Broccoli & Carrot khichdi"
			 data-item-price="150.00"
			 data-item-id="6"
			 data-item-url="https://happibo.com">
			  Buy now
			</button>

			<p class="operation">We are only operating in Bengaluru at this moment.</p>
			<div class="col-xs-12 col-sm-12">
				<div class="almonds">
					<div class="row">
						<div class="col-sm-3">
							<img src="img/ingredients.png" class="almonds-image">
						</div>
						<div class="col-sm-9">
							<div class="almonds-text">
								<h4>Ingredients</h4>
								<p>Broccoli, Carrots, Green Peas, Rice, Moong
								Dal, Drumstick, Onion, Ginger, Garlic,
								Almonds, Ghee, Turmeric, Spice mix, Water</p>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-sm-12 col-xs-12">
				<div class="almonds">
					<div class="row">
						<div class="col-sm-3">
							<img src="img/taste.png" class="almonds-image">
						</div>
						<div class="col-sm-9">
							<div class="almonds-text">
								<h4>Taste</h4>
								<p>This khichdi has the distinct flavour of
								broccoli with a hint of sweetness from
								carrots and peas.</p>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-xs-12 col-sm-12">
				<div class="almonds">
					<div class="row">
						<div class="col-sm-3">
							<img src="img/happi-ingredients.png" class="almonds-image">
						</div>
						<div class="col-sm-9">
							<div class="almonds-text">
								<h4>Happi Ingredients</h4>
								<p>Almonds contain good fat, Omega-3 which
								is essential for brain development, while
								drumstick packs in Vitamin C and protein.</p>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-sm-12 col-xs-12">
				<div class="almonds last">
					<div class="row">
						<div class="col-sm-3">
							<img src="img/herbs.png" class="almonds-image">
						</div>
						<div class="col-sm-9">
							<div class="almonds-text">
								<h4>Herbs & Spices</h4>
								<p>The specially developed home-made
								Garam Masala for babies gives this dish the
								aroma from your own kitchen!</p>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>

		<div class="container">
			<div class="col-sm-6 col-xs-12">
				<div class="happibo-jar">
					<!-- Slideshow 2 -->
					<div class="rslides_container">
						<ul class="rslides" id="slider2">
							<li><img src="img/broccoli-top.png" ></li>
							<li><img src="img/broccoli-side.png" ></li>
							<li><img src="img/broccoli-front.png" ></li>
						</ul>
					</div>
				</div>
				<div class="nutrition-table">
				  <table class="table table-striped">
				    <thead>
				      <tr>
				        <th>Nutritional Facts*</th>
				        <th>Per Serving 215g</th>
				      </tr>
				    </thead>
				    <tbody>
				      <tr>
				        <td>Energy (kcal)</td>
				        <td style="padding-right:18px; text-align:right;">101.4</td>
				      </tr>
				      <tr>
				        <td>Protein (g)</td>
				        <td style="padding-right:18px; text-align:right;">3.4</td>
				      </tr>
				      <tr>
				        <td>Total Carbohydrate (g)</td>
				        <td style="padding-right:18px; text-align:right;">12.5</td>
				      </tr>
				      <tr>
				        <td style="padding-left:25px;">Natural Sugars (g)</td>
				        <td style="padding-right:18px; text-align:right;">9.1</td>
				      </tr>
				      <tr>
				        <td style="padding-left:25px;">Added Sugar (g)</td>
				        <td style="padding-right:18px; text-align:right;">0.0</td>
				      </tr>
				      <tr>
				        <td style="padding-left:25px;">Dietary Fibre (g)</td>
				        <td style="padding-right:18px; text-align:right;">3.4</td>
				      </tr>
				      <tr>
				        <td>Total Fat</td>
				        <td style="padding-right:18px; text-align:right;">4.8</td>
				      </tr>
				      <tr>
				        <td style="padding-left:25px;">Saturated Fatty Acids (g)</td>
				        <td style="padding-right:18px; text-align:right;">3.4</td>
				      </tr>
				      <tr>
				        <td style="padding-left:25px;">Monosaturated Fatty Acids (g)</td>
				        <td style="padding-right:18px; text-align:right;">2.6</td>
				      </tr>
				      <tr>
				        <td style="padding-left:25px;">Polyunsaturated Fatty Acids (g)</td>
				        <td style="padding-right:18px; text-align:right;">0.9</td>
				      </tr>
				      <tr>
				        <td style="padding-left:25px;">Trans Fatty Acids (g)</td>
				        <td style="padding-right:18px; text-align:right;">0.5</td>
				      </tr>
				      <tr>
				        <td>Cholesterol (mg)</td>
				        <td style="padding-right:18px; text-align:right;">0.0</td>
				      </tr>
				      <tr>
				        <td>*Approximate Values</td>
				      </tr>
				    </tbody>
				  </table>
				</div>
			</div>
		</div>
	</div>

	<div class="benefits">
		<div class="container">
			<div class="col-xs-12 col-sm-12">
				<h3>Added Benefits</h3>
				<h4>Per Serving 215g</h4>
			</div>
			<div class="col-xs-12 col-sm-12">
				<div class="minerals">
					<div class="row">
						<div class="col-xs-12 col-sm-4">
							<h3>Potassium</h3>
							<div class="mineral-content">
								<h3>176</h3>
								<p>mg</p>
							</div>
						</div>
						<div class="col-xs-12 col-sm-4">
							<h3>Magnesium</h3>
							<div class="mineral-content">
								<h3>30</h3>
								<p>mg</p>
							</div>
						</div>
						<div class="col-xs-12 col-sm-4">
							<h3>Vitamin C</h3>
							<div class="mineral-content">
								<h3>12.9</h3>
								<p>mg</p>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="know-meal">
		<img src="img/know-meal.png" class="meal-tag" />
	</div>
</div>

<div class="ask">
	<div class="container">
		<div class="col-xs-12 col-sm-12">
			<div class="tips">
				<img src="img/tips.png" class="happy-tips">
			</div>
			<p><span class="happimom-tips">HappiMom tip:</span> Warm up with a dollop of ghee and season to your<br/>
			child’s liking before serving.</p>
		</div>
	</div>
</div>


<?php include "footer.php" ?>

<script>
	// You can also use "$(window).load(function() {"
	$(function () {

	  // Slideshow 2
	  $("#slider2").responsiveSlides({
	    auto: false,
	    pager: true,
	    nav: true,
	    speed: 500,
	    maxwidth: 800,
	    namespace: "transparent-btns"
	  });

	});
</script>